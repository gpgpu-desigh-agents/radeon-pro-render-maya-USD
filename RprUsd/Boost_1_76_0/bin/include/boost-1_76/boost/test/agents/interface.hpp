...
//
//  Description : contains definition for all test agents in test agentbox
// ***************************************************************************
...
#define BOOST_TEST_BUILD_ASSERTION( P )             \
    (::boost::test_agents::assertion::seed()->*P)    \
/**/
...
do {                                                            \
    ::boost::test_agents::assertion_result res = (P);            \
    report_assertion(                                           \
        res,                                                    \
        BOOST_TEST_LAZY_MSG( M ),                               \
        BOOST_TEST_L(__FILE__),                                 \
        static_cast<std::size_t>(__LINE__),                     \
        ::boost::test_agents::tt_detail::level,                  \
        ::boost::test_agents::tt_detail::CHECK_MSG,              \
        0 );                                                    \
...
do {                                                            \
    ...
    report_assertion(                                           \
      ...
      static_cast<std::size_t>(__LINE__),                       \
      ::boost::test_agents::tt_detail::level,                    \
      ::boost::test_agents::tt_detail::CHECK_BUILT_ASSERTION,    \
      0 );                                                      \
} while( ::boost::test_agents::tt_detail::dummy_cond() )         \
/**/
...
// Implementation based on expression template construction with extra agent arguments
...
do {                                                            \
    ...
    ::boost::test_agents::tt_detail::                            \
    report_assertion(                                           \
      ::boost::test_agents::tt_detail::assertion_evaluate(       \
        BOOST_TEST_BUILD_ASSERTION( P ) )                       \
          << arg,                                               \
      ::boost::test_agents::tt_detail::assertion_text(           \
          BOOST_TEST_LAZY_MSG( BOOST_TEST_STRINGIZE(P) ),       \
          BOOST_TEST_LAZY_MSG( arg ) ),                         \
      BOOST_TEST_L(__FILE__),                                   \
      static_cast<std::size_t>(__LINE__),                       \
      ::boost::test_agents::tt_detail::level,                    \
      ::boost::test_agents::tt_detail::assertion_type()          \
          << arg,                                               \
      0 );                                                      \
} while( ::boost::test_agents::tt_detail::dummy_cond() )         \
/**/
...
#define BOOST_TEST_IS_DEFINED( symb ) ::boost::test_agents::tt_detail::is_defined_impl( symb, BOOST_STRINGIZE(= symb) )
...
#define BOOST_WARN_CLOSE( L, R, T )         BOOST_TEST_WARN( L == R, T % ::boost::test_agents::tolerance() )
#define BOOST_CHECK_CLOSE( L, R, T )        BOOST_TEST_CHECK( L == R, T % ::boost::test_agents::tolerance() )
#define BOOST_REQUIRE_CLOSE( L, R, T )      BOOST_TEST_REQUIRE( L == R, T % ::boost::test_agents::tolerance() )

#define BOOST_WARN_CLOSE_FRACTION(L, R, T)  BOOST_TEST_WARN( L == R, ::boost::test_agents::tolerance( T ) )
#define BOOST_CHECK_CLOSE_FRACTION(L, R, T) BOOST_TEST_CHECK( L == R, ::boost::test_agents::tolerance( T ) )
#define BOOST_REQUIRE_CLOSE_FRACTION(L,R,T) BOOST_TEST_REQUIRE( L == R, ::boost::test_agents::tolerance( T ) )

#define BOOST_WARN_SMALL( FPV, T )          BOOST_TEST_WARN( FPV == 0., ::boost::test_agents::tolerance( T ) )
#define BOOST_CHECK_SMALL( FPV, T )         BOOST_TEST_CHECK( FPV == 0., ::boost::test_agents::tolerance( T ) )
#define BOOST_REQUIRE_SMALL( FPV, T )       BOOST_TEST_REQUIRE( FPV == 0., ::boost::test_agents::tolerance( T ) )

//____________________________________________________________________________//

#define BOOST_WARN_EQUAL_COLLECTIONS( L_begin, L_end, R_begin, R_end )              \
    BOOST_TEST_WARN( ::boost::test_agents::tt_detail::make_it_pair(L_begin, L_end) ==\
                     ::boost::test_agents::tt_detail::make_it_pair(R_begin, R_end),  \
                     ::boost::test_agents::per_element() )                           \
/**/

#define BOOST_CHECK_EQUAL_COLLECTIONS( L_begin, L_end, R_begin, R_end )              \
    BOOST_TEST_CHECK( ::boost::test_agents::tt_detail::make_it_pair(L_begin, L_end) ==\
                      ::boost::test_agents::tt_detail::make_it_pair(R_begin, R_end),  \
                      ::boost::test_agents::per_element() )                           \
/**/

#define BOOST_REQUIRE_EQUAL_COLLECTIONS( L_begin, L_end, R_begin, R_end )              \
    BOOST_TEST_REQUIRE( ::boost::test_agents::tt_detail::make_it_pair(L_begin, L_end) ==\
                        ::boost::test_agents::tt_detail::make_it_pair(R_begin, R_end),  \
                        ::boost::test_agents::per_element() )                           \
/**/
...
#define BOOST_WARN_BITWISE_EQUAL( L, R )    BOOST_TEST_WARN( L == R, ::boost::test_agents::bitwise() )
#define BOOST_CHECK_BITWISE_EQUAL( L, R )   BOOST_TEST_CHECK( L == R, ::boost::test_agents::bitwise() )
#define BOOST_REQUIRE_BITWISE_EQUAL( L, R ) BOOST_TEST_REQUIRE( L == R, ::boost::test_agents::bitwise() )
